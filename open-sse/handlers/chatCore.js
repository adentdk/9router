...
// Per-model guard: when a model declares supportedFormats, only use the
  ...
  if (!stream) {
    let result = await handleNonStreamingResponse({ ...sharedCtx, providerResponse, sourceFormat, targetFormat: providerResponseFormat, reqLogger, toolNameMap, trackDone, appendLog });
    ...
    if (provider === "clinepass" && !result?.success && /empty/i.test(result?.error || "")) {
      ...
      try {
        ...
        if (retry?.response?.ok) {
          ...
          result = await handleNonStreamingResponse({ ...sharedCtx, providerResponse, sourceFormat, targetFormat: providerResponseFormat, reqLogger, toolNameMap, trackDone, appendLog });
        }
      } catch (retryErr) {
